...
//  Companion
//
//  Created by Santosh Naidu on 14/11/22.
//
...
public class MessageViewModel {
    ...
    public init(bubbleStyle: BubbleStyle) {
...
//        }
    ...
    private func getDataFromFile<T>(fileName: String, completion: (_ data: [T], _ pagination: Pagination?) -> ()) where T: Decodable {
        let bundle = Bundle(for: MessageViewModel.self)
        guard let jsonData = Data.dataFromJSONFile(fileName,bundle: bundle) else {
            completion([], nil)
            return
        }
